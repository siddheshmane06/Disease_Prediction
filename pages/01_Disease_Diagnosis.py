...
def fetch_disease_info(disease=None):
    ...
    # print("Disease variable:", disease)  # Add this line for debugging
    ...
    # Now proceed with the 'disease' key checking
    if disease not in disease_data:
        print(f"Disease {disease} not found in data")
        return None  # Handle the error as appropriate
    
    common_name = disease_data[disease].get("common name")
    disease_overview = disease_data[disease].get("Disease overview", {})
    gen_overview = disease_overview[0].get("general_overview") if disease_overview else None
    frequency = disease_overview[1].get("frequency") if len(disease_overview) > 1 else None
    gen_info = disease_overview[2].get("general_info") if len(disease_overview) > 2 else None
    causes = disease_overview[2].get("cause") if len(disease_overview) > 2 else None
    ...
    types = disease_data[disease].get("types")
...
def disease_app():
    ...
    if len(inp_symptoms) >= 4:
        if submit:
            ...
            # st.markdown("---")
            # if len(gen_info_lst) > 3:
            #     st.markdown(f"🧬 {gen_info_lst[0]}")
            #     st.markdown(f"💊 {gen_info_lst[1]}")
            #     st.markdown(f"🌡️ {gen_info_lst[2]}")
            #     st.markdown(f"🕒 {gen_info_lst[3]}")
            # else:
            #     st.markdown("Information for the disease is not complete.")
            if len(gen_info_lst) > 0:
                st.markdown(f"🧬 {gen_info_lst[0]}")
            if len(gen_info_lst) > 1:
                st.markdown(f"💊 {gen_info_lst[1]}")
            if len(gen_info_lst) > 2:
                st.markdown(f"🌡️ {gen_info_lst[2]}")
            if len(gen_info_lst) > 3:
                st.markdown(f"🕒 {gen_info_lst[3]}")
            st.markdown("---")
